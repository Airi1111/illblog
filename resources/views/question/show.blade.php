...
<body>
    <x-app-layout>
        ...
        <h1 textalign="center"class="question_title">Comments for Question</h1>
    ...
    <script>
        ...
        $('#commentForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('questioncomments.store') }}",
                data: $(this).serialize(),
                success: function(response) {
                    // 新しいコメントをリストに追加
                    $('#commentList').append('<li>' + response.user.name + ': ' + response.comment.comment + '</li>');
                    // フォームをクリア
    ...
    function deleteComment(commentId) {
        if (confirm('本当に削除しますか？')) {
            $.ajax({
                type: 'DELETE',
                url: "{{ url('/questioncomments') }}/" + commentId,
                success: function(response) {
                    if (response.success) {
                        // 削除成功後、コメントをリストから削除
